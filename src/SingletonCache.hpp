// https://github.com/EclipseMenu/EclipseMenu/blob/e401610c99674ded967e62cc07a928ce5d5615ce/src/modules/utils/SingletonCache.hpp#L1
#pragma once
...
namespace fast {
    ...
    base_type<T>* getNested() {
        if constexpr (std::is_same_v<base_type<T>, EditorUI>) {
            if (auto editor = fast::get<LevelEditorLayer>()) {
                return editor->m_editorUI;
            }
        } else if constexpr (std::is_same_v<base_type<T>, UILayer>) {
            if (auto gjbgl = fast::get<GJBaseGameLayer>()) {
    ...
    base_type<T>* get() {
        ...
        if constexpr (NestedInstance<type>) {
            ...
            if constexpr (std::is_same_v<type, PlayLayer>) {
                ref = &fast::get<GameManager>()->m_playLayer;
            } else if constexpr (std::is_same_v<type, LevelEditorLayer>) {
                ref = &fast::get<GameManager>()->m_levelEditorLayer;
            } else if constexpr (std::is_same_v<type, GJBaseGameLayer>) {
                ref = &fast::get<GameManager>()->m_gameLayer;
            } else if constexpr (std::is_same_v<type, MenuLayer>) {
                ref = &fast::get<GameManager>()->m_menuLayer;
